...
class NSGA2:
    ...
    def error_func2(self, a):
        ...
        return [
            np.amax(abs(X1[0, self.cfg.CALC.Nte + 1 :])) * 10 ** 3,
            np.amax(abs(X2[0, self.cfg.CALC.Nte + 1 :])) * 10 ** 3,
    ...
    def run(self, pid):
        ...
        problem = Problem(self.cfg.COMM.PARAM, self.cfg.NSGA2.OBJECT)  # 最適化パラメータの数, 目的関数の数
...
te{self.cfg.CALC.TE_str}_se{self.cfg.CALC.SE_str}.csv"
            ...
            np.savetxt(
                self.cfg.DATA.DIR
                + f"param/{pid}_param_nsga2_{self.cfg.COMM.MODE}_\
te{self.cfgCALC.TE_str}_se{self.cfg.CALC.SE_str}.csv",
                params[index, :],
                delimiter=",",
            )
            logger.info(
